...
class CI_DB_pdo_driver extends CI_DB {
	...
	function _execute($sql)
	{
		...
		if (is_object($result_id) && $result_id->execute())
		{
			...
			if (stripos($sql, 'SELECT ') === 0)//!!!Fixed: repeat to insert the data when sql contains "select" word, modified by hhxsv5@sina.com
			{
				//SET affect_rows = 0 if SELECT SQL
				$this->affect_rows = 0;//count($result_id->fetchAll());//!!!PDOStatment::fetchAll can only fetch once
			}
			else
			{
	...
	{
		return trim($sql);//Modified by hhxsv5@sina.com, trim the white blank
	}
